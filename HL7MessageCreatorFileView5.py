...
Author: J.M.Thomas"""
    ...
    def setup_creator(self):
        # Title (centered)
        title_container = tk.Frame(self.content_frame, bg=BG_COLOR)
        ...
        self.create_button = tk.Button(
            self.creator_content_frame, text="Create", command=self.create_patient, bg=BG_COLOR, fg=TEXT_COLOR, font=("Arial", 14), activebackground="#3A3C5A"
        )
    ...
    def on_frame_configure(self, event=None):
        # Updated to ensure scroll region adjusts correctly, reducing gaps
        self.canvas.configure(scrollregion=self.canvas.bbox("all"))
        self.canvas.update_idletasks()  # Force layout update
    ...
    def populate_procedure_tree(self, filter_text="", expand_to_item=None):
        for item in self.tree.get_children():
                ...
                for _, proc in procs.iterrows():
                    ...
                    if not filter_text or (
                        ...
                        if not cat_has_match:
                            ...
                            cat_has_match = True
                        proc_id = self.tree.insert(cat_id, tk.END, text=proc_text, values=(proc["name"], proc["id"], proc["description"], proc["special_needs"], proc["cpt"]))
                        if not first_match and filter_text:
                            first_match = (spec_id, cat_id, proc_id)
        if first_match and expand_to_item:
            spec_id, cat_id, proc_id = first_match
            self.tree.item(spec_id, open=True)
            self.tree.item(cat_id, open=True)
            self.tree.see(proc_id)

    def filter_procedures(self):
        filter_text = self.search_var.get()
        self.populate_procedure_tree(filter_text, expand_to_item=True)
    ...
    def choose_random_procedure(self):
        # Updated to randomize the last existing procedure instead of adding a new one
        if 0 <= self.current_patient_index < len(self.patients):
            proc = self.procedures.sample(1).iloc[0]
            patient = self.patients[self.current_patient_index]
            if patient["procedures"]:  # If additional procedures exist
                last_proc = patient["procedures"][-1]
                last_proc["{procedure}"].set(proc["name"])
                last_proc["{procedureId}"].set(proc["id"])
                last_proc["{procedureDescription}"].set(proc["description"])
                last_proc["{specialNeeds}"].set(proc["special_needs"])
            else:  # Target the base procedure
    ...
    def random_patient(self):
        # Updated to set existing StringVar objects, preserving traces for preview updates
        if 0 <= self.current_patient_index < len(self.patients):
    ...
    def add_procedure(self):
        proc = {
            "{procedure}": tk.StringVar(value=""),
            "{procedureDescription}": tk.StringVar(value=""),
            "{specialNeeds}": tk.StringVar(value=""),
            "{procedureId}": tk.StringVar(value="")
        }
        self.patients[self.current_patient_index]['procedures'].append(proc)
        self.add_procedure_fields(proc)
        self.creator_update_preview()
        # Update layout to prevent gaps
        self.procedures_frame.update_idletasks()
    ...
    def remove_last_procedure(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            else:  # Reset base procedure if no additional ones
                patient['base_vars']['{procedure}'].set("")
                patient['base_vars']['{procedureDescription}'].set("")
                patient['base_vars']['{specialNeeds}'].set("")
                patient['base_vars']['{procedureId}'].set("")
                patient['base_vars']['{cptCode}'].set("")
            self.creator_update_preview()
            # Update layout to prevent gaps
            self.procedures_frame.update_idletasks()
            self.canvas.update_idletasks()
    ...
    def creator_update_preview(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            # Only replace variables with non-empty values
            for key, val in base_values.items():
                if val:  # Check if the value is non-empty
...
base_prompts = [
    ...
    {"key": "{scheduledTime}", "prompt": "Setup time of the procedure: (HHMMSS)"},
