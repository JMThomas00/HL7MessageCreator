...
# Default HL7 template (corrected typo: {encounterType}} to {encounterType})
default_hl7 = """
...
class HL7MessageApp:
    ...
    def setup_base_prompts(self):
        # Initialize list to store encounter type radio buttons
        ...
        for prompt in base_prompts:
            ...
            if prompt['key'] == "{encounterType}":
                # Create radio buttons for encounter type instead of text entry
                ...
                # Standard text entry for all other prompts
    ...
    def random_staff(self):
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            # Calculate total staff roles: fixed roles (Circulator, Scrub, CRNA, Anesthesiologist) + additional staff
    ...
    def clear_all(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            for var in patient['base_vars'].values():
                var.set("")
            # Reset encounterType to default "IP" after clearing
    ...
    def create_new_patient(self):
        if self.mode != "Creator":
            ...
            return
        # Set encounterType to "IP" by default for new patients
        patient = {
    ...
    def creator_load_patient(self):
        patient = self.patients[self.current_patient_index]
        # Link encounter type radio buttons to the current patient's base_vars["{encounterType}"]
    ...
    def creator_update_preview(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            base_values["{specialty}"] = patient['procedure_specialty'].get()  # Add specialty for PV1 and AIP
    ...
    def add_procedure_segments(self, template, proc_num, proc_values):
        ...
        end_idx = next(i for i, line in enumerate(lines) if line.startswith("NTE|2|")) + 1  # Include NTE|2|
        proc_block = lines[start_idx:end_idx]  # AIS and NTE only (no AIL)
        new_block = []
