...
class HL7MessageApp:
    def __init__(self, root):
        ...
        self.file_menu.add_command(label="New Patient", command=self.create_new_patient)
        self.file_menu.add_command(label="Open File(s)", command=self.open_files)
    ...
    def set_mode(self, mode):
        ...
        if mode == "Creator":
            self.root.title("HL7 Message Creator")
            self.file_menu.entryconfig("New Patient", state="normal")
            self.file_menu.entryconfig("Open File(s)", state="disabled")
            self.setup_creator()
        elif mode == "Editor":
            self.root.title("HL7 Message Editor")
            self.file_menu.entryconfig("New Patient", state="disabled")
            self.file_menu.entryconfig("Open File(s)", state="normal")
            self.setup_editor()
    ...
    def open_help(self):
        ...
        **Creator Mode**
        ...
        - "Create" generates messages; File > Save to save.

        **Editor Mode**
        - File > Open File(s) to load .hl7 files.
        - Navigate with "Previous" and "Next".
        - Edit manually or "Randomize All".
        - File > Save to save edited messages.
        """
