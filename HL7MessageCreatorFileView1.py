...
class HL7MessageApp:
    def __init__(self, root):
        ...
        # Attempt to set a dark theme for the application window (may not affect title bar on all systems)
        try:
            self.root.tk.call("tk", "scaling", 1.0)  # Ensure proper scaling
            # Set dark theme for ttk widgets
            style = ttk.Style()
            style.theme_use('clam')  # Use a theme we can customize
            style.configure("TFrame", background=BG_COLOR)
            style.configure("TLabel", background=BG_COLOR, foreground=TEXT_COLOR)
            style.configure("TButton", background=BG_COLOR, foreground=TEXT_COLOR, activebackground="#3A3C5A")
            # Style for Treeview
...
Author: J.M.Thomas"""
    ...
    def setup_creator(self):
        ...
        # Right-side content frame with scrollbar
        ...
        self.scrollbar = ttk.Scrollbar(self.creator_content_container, orient=tk.VERTICAL, command=self.canvas.yview)
        ...
        self.creator_preview_text = scrolledtext.ScrolledText(
            self.creator_content_frame, width=160, height=20, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR
        )
        ...
        self.creator_prev_button = tk.Button(
            self.creator_button_frame, text="Previous", command=self.creator_prev_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        )
        self.creator_prev_button.pack(side=tk.LEFT, padx=5)
        self.creator_next_button = tk.Button(
            self.creator_button_frame, text="Next", command=self.creator_next_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        )
        self.creator_next_button.pack(side=tk.LEFT, padx=5)

        # Command buttons
        tk.Button(
            self.creator_button_frame, text="Browse Procedures", command=self.toggle_procedure_browser, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            self.creator_button_frame, text="Create New Patient", command=self.create_new_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            self.creator_button_frame, text="Quit", command=self.quit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            self.creator_button_frame, text="Save & Exit", command=self.creator_save_and_exit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(
            self.creator_content_frame, text="Add Procedure", command=self.add_procedure, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(pady=5)
        tk.Button(
            self.creator_content_frame, text="Add Staff Member", command=self.add_staff_member, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(pady=5)
        ...
        tk.Button(
            random_frame, text="Random Patient", command=self.random_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            random_frame, text="Random Surgeon", command=self.random_surgeon, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            random_frame, text="Random Staff", command=self.random_staff, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)

        # Create button
        self.create_button = tk.Button(
            self.creator_content_frame, text="Create", command=self.create_patient, bg=BG_COLOR, fg=TEXT_COLOR, font=("Arial", 16), activebackground="#3A3C5A"
        )
    ...
    def setup_procedure_browser(self):
        ...
        tk.Button(
            button_frame, text="Choose Random", command=self.choose_random_procedure, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            button_frame, text="Close", command=self.toggle_procedure_browser, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
    ...
    def setup_editor(self):
        ...
        self.editor_preview_text = scrolledtext.ScrolledText(
            self.editor_content_frame, width=80, height=20, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR
        )
        ...
        self.editor_prev_button = tk.Button(
            self.editor_button_frame, text="Previous", command=self.editor_prev_block, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        )
        self.editor_prev_button.pack(side=tk.LEFT, padx=5)
        self.editor_next_button = tk.Button(
            self.editor_button_frame, text="Next", command=self.editor_next_block, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        )
        self.editor_next_button.pack(side=tk.LEFT, padx=5)
        tk.Button(
            self.editor_button_frame, text="Randomize All", command=self.editor_randomize_all, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            self.editor_button_frame, text="Quit", command=self.quit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
        tk.Button(
            self.editor_button_frame, text="Save and Exit", command=self.editor_save_and_exit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
        ).pack(side=tk.LEFT, padx=5)
    ...
    def editor_manual_edit_block(self):
        if 0 <= self.current_block < len(self.patient_blocks):
            ...
            random_patient_button = tk.Button(
                self.editor_edit_frame, text="Random Patient", command=self.editor_random_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A"
            )
