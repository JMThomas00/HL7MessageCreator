...
class HL7MessageApp:
    ...
    def open_help(self):
        ...
        **Creator Mode**
        ...
        - Select message types (S12, S14, S15) via checkboxes.
        ...
        **Editor Mode**
        ...
        - File > Save (Ctrl+S) to save edited messages.
        """
    ...
    def open_files(self):
        ...
        if files:
            ...
            for patient_name, patient_files in patient_groups.items():
                patient_files.sort()
                self.patient_blocks.append(("Custom", patient_name, patient_files))
    ...
    def editor_save_files(self):
        ...
        if not out_dir:
            return
        total_messages = len(self.edited_messages)
    ...
    def setup_creator(self):
        ...
        # Message type selection checkboxes
        message_type_frame = tk.Frame(self.creator_button_frame_top, bg=BG_COLOR)
        message_type_frame.pack(side=tk.RIGHT)
        tk.Label(message_type_frame, text="Generate:", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT).pack(side=tk.LEFT, padx=5)
        self.s12_var = tk.IntVar(value=1)  # Default checked
        s12_check = tk.Checkbutton(message_type_frame, text="S12", variable=self.s12_var, fg=TEXT_COLOR, bg=BG_COLOR, selectcolor=PREVIEW_BG, font=DEFAULT_FONT)
        s12_check.pack(side=tk.LEFT, padx=2)
        tk.Label(message_type_frame, text="|", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT).pack(side=tk.LEFT)
        self.s14_var = tk.IntVar(value=1)  # Default checked
        s14_check = tk.Checkbutton(message_type_frame, text="S14", variable=self.s14_var, fg=TEXT_COLOR, bg=BG_COLOR, selectcolor=PREVIEW_BG, font=DEFAULT_FONT)
        s14_check.pack(side=tk.LEFT, padx=2)
        tk.Label(message_type_frame, text="|", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT).pack(side=tk.LEFT)
        self.s15_var = tk.IntVar(value=0)  # Default unchecked
        s15_check = tk.Checkbutton(message_type_frame, text="S15", variable=self.s15_var, fg=TEXT_COLOR, bg=BG_COLOR, selectcolor=PREVIEW_BG, font=DEFAULT_FONT)
        s15_check.pack(side=tk.LEFT, padx=2)
...
def create_new_patient(self):
    ...
    def calculate_event_time(self, base_time_str, offset_minutes):
        """Calculate event time by adding offset minutes to base time."""
        if base_time_str == "{scheduledTime}" or not base_time_str:
            return "{eventTime}"
        try:
            hours = int(base_time_str[0:2])
            mins = int(base_time_str[2:4])
            secs = int(base_time_str[4:6]) if len(base_time_str) >= 6 else 0
            base_dt = datetime(2000, 1, 1, hours, mins, secs)
            delta = timedelta(minutes=offset_minutes)
            event_dt = base_dt + delta
    ...
    def create_s15_message(self, s12_msg):
        """Create an S15 message by modifying the S12 message."""
        s15_msg = s12_msg
        # Set MSH-9 to SIU^S15
        s15_msg = re.sub(r'(\|SIU\^S12\|)', '|SIU^S15|', s15_msg)
        # Set SCH-25 to CANCELLED
        lines = s15_msg.splitlines()
        for i, line in enumerate(lines):
            if line.startswith("SCH|"):
                parts = line.split("|")
                while len(parts) < 26:  # Ensure SCH-25 exists
                    parts.append("")
                parts[25] = "CANCELLED"
                lines[i] = "|".join(parts)
                break
    ...
    def build_event_messages(self, template, base_values, duration_min):
        """Generate S12 and S14 messages with correct event times and MSH-9 settings."""
        s12_template = "\n".join(line for line in template.splitlines() if not line.startswith("OBX"))
        scheduled_time = base_values.get("{scheduledTime}", "{scheduledTime}")
        arrive_offset = -60  # Arrive event offset for S12
        arrive_time = self.calculate_event_time(scheduled_time, arrive_offset)
        messages = []

        # Generate S12 message if selected
        if self.s12_var.get():
            s12_replacements = base_values.copy()
            s12_replacements["{eventTime}"] = arrive_time
            s12_msg = self.fill_template(s12_template, s12_replacements)
            messages.append((s12_msg, "00"))

        # Generate S14 messages if selected
        if self.s14_var.get():
            for i, (event_name, offset) in enumerate(case_events):
                if isinstance(offset, str):
                    # Handle special offsets like "duration-30"
                    if "duration" in offset:
                        duration = duration_min
                        offset = duration - int(offset.split("-")[1])
                    elif "exiting" in offset:
                        # Approximate for simplicity; actual time depends on prior events
                        duration = duration_min
                        exiting_offset = duration - 10  # Base exiting offset
                        delta = int(offset.split("exiting")[1])
                        offset = exiting_offset + delta
                    elif "in_pacu" in offset:
                        # Approximate; actual time depends on in_pacu
                        duration = duration_min
                        in_pacu_offset = duration - 5  # Base in_pacu offset
                        delta = int(offset.split("+")[1])
                        offset = in_pacu_offset + delta
                event_time = self.calculate_event_time(scheduled_time, offset)
                event_replacements = base_values.copy()
                event_replacements["{eventTime}"] = event_time
                event_replacements["{caseEvent}"] = event_name
                event_msg = self.fill_template(template, event_replacements)
                # Set MSH-9 to SIU^S14 for event messages
                event_msg = re.sub(r'(\|SIU\^S12\|)', '|SIU^S14|', event_msg)
                messages.append((event_msg, f"{i+1:02}"))

        # Generate S15 message if selected
        if self.s15_var.get() and messages and messages[0][1] == "00":
            s15_msg = self.create_s15_message(messages[0][0])
            messages.append((s15_msg, f"{len(messages):02}"))

        return messages

    def create_patient(self):
        """Generate HL7 messages based on selected message types."""
        if not (self.s12_var.get() or self.s14_var.get() or self.s15_var.get()):
            messagebox.showwarning("No Messages Selected", "Please select at least one message type to generate.")
            return
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            template = self.build_template(patient)
                ...
                if instance:
                    ...
                    for i, line in enumerate(lines):
                        if instance in line and var in line:
                            lines[i] = lines[i].replace(var, value)
                    message = '\n'.join(lines)
                else:
                    message = message.replace(var, value)
        message = message.replace("{primaryFirstName}", primary_fname)
        message = message.replace("{primaryLastName}", primary_lname)
        for i, (last, first) in enumerate(staff_names, start=1):
            message = message.replace(f"{{lastName{i}}}", last)
            message = message.replace(f"{{firstName{i}}}", first)
        return message

    def editor_randomize_all(self):
        if self.current_block < 0 or not self.patient_blocks:
            return
        block = self.patient_blocks[self.current_block]
        files = block[2]
        name = self.surgeon_names.sample(1).iloc[0]
        primary_fname = name["First Name"]
        primary_lname = name["Last Name"]
        staff_count = max([int(re.search(r'\{lastName(\d+)\}', f).group(1)) for f in files if re.search(r'\{lastName\d+\}', f)] + [0])
        staff_names = []
        for _ in range(staff_count):
            name = self.staff_names.sample(1).iloc[0]
            staff_names.append((name["Last Name"], name["First Name"]))
        for file_path in files:
            with open(file_path, 'r') as f:
                message = f.read()
            message = self.replace_variables(message, primary_fname, primary_lname, staff_names)
            self.edited_messages[file_path] = message
        self.editor_update_preview()
    ...
    def editor_update_preview(self):
        ...
        if self.current_block < 0 or not self.patient_blocks:
            ...
            return
        block = self.patient_blocks[self.current_block]
        folder, patient_name, files = block
        tk.Label(self.editor_prompt_frame, text=f"Folder: {folder}", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT).pack(side=tk.LEFT, padx=5)
        tk.Label(self.editor_prompt_frame, text=f"Patient: {patient_name}", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT).pack(side=tk.LEFT, padx=5)
        tk.Label(self.editor_prompt_frame, text="Variables:", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT).pack(side=tk.LEFT, padx=5)
        variables = set()
        for file_path in files:
            with open(file_path, 'r') as f:
                message = f.read()
            for match in re.finditer(r'\{(\w+)\}', message):
                var = match.group(1)
                instance = None
                if var in ["lastName", "firstName"] and not var.endswith("Name"):
                    continue
                for line in message.split('\n'):
                    if match.group(0) in line:
                        instance = line.split('|')[0]
                        break
                variables.add((var, instance))
        variables = sorted(variables, key=lambda x: (x[1] or '', x[0]))
        for var, instance in variables:
            frame = tk.Frame(self.editor_edit_frame, bg=BG_COLOR)
            frame.pack(fill=tk.X, pady=2)
            tk.Label(frame, text=f"{var} ({instance or 'All'}):", fg=TEXT_COLOR, bg=BG_COLOR, font=DEFAULT_FONT, width=30, anchor="w").pack(side=tk.LEFT, padx=5)
            entry = UppercaseEntry(frame, base_width=20, min_width=10, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR)
            entry.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
            self.edit_fields.append((entry, var, instance))
            self.entry_widgets.append(entry)
            entry.bind("<KeyRelease>", self.editor_manual_edit)
        first_file = files[0]
        message = self.edited_messages.get(first_file, None)
        if not message:
            with open(first_file, 'r') as f:
                message = f.read()
        self.editor_preview_text.insert(tk.END, message)
        total = len(self.patient_blocks)
        idx = self.current_block
        self.editor_prev_button.config(state="disabled" if idx <= 0 else "normal", fg=DITHERED_TEXT if idx <= 0 else TEXT_COLOR)
        self.editor_next_button.config(state="disabled" if idx >= total - 1 else "normal", fg=DITHERED_TEXT if idx >= total - 1 else TEXT_COLOR)

    def editor_manual_edit(self, event):
        if self.current_block < 0 or not self.patient_blocks:
            return
        block = self.patient_blocks[self.current_block]
        files = block[2]
        manual_values = {}
        for entry, var, instance in self.edit_fields:
            value = entry.get()
            if value:
                manual_values[(var, instance)] = value
        name = self.surgeon_names.sample(1).iloc[0]
        primary_fname = name["First Name"]
        primary_lname = name["Last Name"]
        staff_count = max([int(re.search(r'\{lastName(\d+)\}', f).group(1)) for f in files if re.search(r'\{lastName\d+\}', f)] + [0])
        ...
        for file_path in files:
            with open(file_path, 'r') as f:
                message = f.read()
            message = self.replace_variables(message, primary_fname, primary_lname, staff_names, manual_values)
            self.edited_messages[file_path] = message
        self.editor_update_preview()
    ...
    def editor_next_block(self):
        if self.current_block < len(self.patient_blocks) - 1:
            self.current_block += 1
            self.editor_update_preview()
        elif self.current_block == -1 and self.patient_blocks:
            self.current_block = 0
            self.editor_update_preview()

# Base prompts for Creator mode
base_prompts = [
    ...
    {"key": "{YYYYMMDD}", "prompt": "Schedule Date (YYYYMMDD)"},
    {"key": "{scheduledTime}", "prompt": "Schedule Time (HHMMSS)"},
    {"key": "{duration}", "prompt": "Duration (minutes)"},
    {"key": "{procedure}", "prompt": "Procedure"},
    ...
    {"key": "{locationOR}", "prompt": "Location OR"},
    {"key": "{locationDepartment}", "prompt": "Location Department"}
]

# Procedure fields for additional procedures
procedure_fields = [
    {"key": "{procedure}", "prompt": "Procedure"},
    {"key": "{procedureId}", "prompt": "Procedure ID"},
    {"key": "{procedureDescription}", "prompt": "Procedure Description"},
