...
class HL7MessageApp:
    def __init__(self, root):
        self.root = root
        self.root.title("HL7 Message Creator and Editor")
        ...
        tk.Label(title_frame, text=" Creator & Editor", font=("Georgia", 32), fg=TITLE_CREATOR, bg=BG_COLOR).pack(side=tk.LEFT)
    ...
    def open_help(self):
        ...
        help_text = scrolledtext.ScrolledText(help_window, width=80, height=30, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR)
...
Author: J.M.Thomas"""
    ...
    def setup_creator(self):
        ...
        self.creator_main_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        # Content frame
        self.creator_content_frame = tk.Frame(self.creator_main_container, bg=BG_COLOR)
        self.creator_content_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # Preview text box
        self.creator_preview_text = scrolledtext.ScrolledText(self.creator_content_frame, width=160, height=20, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR)
        self.creator_preview_text.pack(fill=tk.BOTH, expand=True, pady=(0, 10))

        # Button frame
        self.creator_button_container = tk.Frame(self.creator_content_frame, bg=BG_COLOR)
        self.creator_button_container.pack(fill=tk.X)
        self.creator_button_frame = tk.Frame(self.creator_button_container, bg=BG_COLOR)
        self.creator_button_frame.pack(pady=5)

        # Navigation buttons
        self.creator_prev_button = tk.Button(self.creator_button_frame, text="Previous", command=self.creator_prev_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A")
        self.creator_prev_button.pack(side=tk.LEFT, padx=5)
        self.creator_next_button = tk.Button(self.creator_button_frame, text="Next", command=self.creator_next_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A")
        self.creator_next_button.pack(side=tk.LEFT, padx=5)

        # Command buttons
        tk.Button(self.creator_button_frame, text="Browse Procedures", command=self.toggle_procedure_browser, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(self.creator_button_frame, text="Create New Patient", command=self.create_new_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(self.creator_button_frame, text="Quit", command=self.quit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(self.creator_button_frame, text="Save & Exit", command=self.creator_save_and_exit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(self.creator_content_frame, text="Add Procedure", command=self.add_procedure, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(pady=5)
        tk.Button(self.creator_content_frame, text="Add Staff Member", command=self.add_staff_member, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(pady=5)

        # Random buttons
        random_frame = tk.Frame(self.creator_content_frame, bg=BG_COLOR)
        random_frame.pack(fill=tk.X, pady=5)
        tk.Button(random_frame, text="Random Patient", command=self.random_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(random_frame, text="Random Surgeon", command=self.random_surgeon, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(random_frame, text="Random Staff", command=self.random_staff, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)

        # Create button
        self.create_button = tk.Button(self.creator_content_frame, text="Create", command=self.create_patient, bg=BG_COLOR, fg=TEXT_COLOR, font=("Arial", 16), activebackground="#3A3C5A")
    ...
    def setup_editor(self):
        ...
        self.editor_preview_text = scrolledtext.ScrolledText(self.editor_content_frame, width=80, height=20, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR)
        ...
        self.editor_prev_button = tk.Button(self.editor_button_frame, text="Previous", command=self.editor_prev_block, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A")
        self.editor_prev_button.pack(side=tk.LEFT, padx=5)
        self.editor_next_button = tk.Button(self.editor_button_frame, text="Next", command=self.editor_next_block, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A")
        self.editor_next_button.pack(side=tk.LEFT, padx=5)
        tk.Button(self.editor_button_frame, text="Randomize All", command=self.editor_randomize_all, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(self.editor_button_frame, text="Quit", command=self.quit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
        tk.Button(self.editor_button_frame, text="Save and Exit", command=self.editor_save_and_exit, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A").pack(side=tk.LEFT, padx=5)
    ...
    def editor_show_prompt(self, message, options, callback):
        for widget in self.editor_prompt_frame.winfo_children():
            widget.destroy()
        tk.Label(self.editor_prompt_frame, text=message, fg=TEXT_COLOR, bg=BG_COLOR).pack(side=tk.LEFT, padx=5)
        for option in options:
            tk.Button(self.editor_prompt_frame, text=option, command=lambda o=option: callback(o),
    ...
    def editor_manual_edit_block(self):
        if 0 <= self.current_block < len(self.patient_blocks):
            ...
            random_patient_button = tk.Button(self.editor_edit_frame, text="Random Patient", command=self.editor_random_patient, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A")
