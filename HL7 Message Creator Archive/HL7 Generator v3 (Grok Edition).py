...
# Default HL7 template (unchanged)
default_hl7 = """
...
# Updated case events with modified PACU-related offsets (unchanged)
case_events = [
    ("arrive", -60),
    ("in_preop", -45),
    ("out_preop", -15),
...
def prompt_user(args):
    ...
    if args.date:
        ...
        values["{YYYYMMDD}"] = val if val else "20230101"

    # Updated to include seconds in prompt and format
    if args.time:
        values["{scheduledTime}"] = args.time.replace(":", "")
    else:
        val = input("What's the setup time of the procedure? (Format is HH:MM:SS)? ").strip()
        # Default to 08:00:00 if blank, pad with zeros if incomplete
        if not val:
            values["{scheduledTime}"] = "080000"
        else:
            parts = val.split(":")
            while len(parts) < 3:
                parts.append("00")
            values["{scheduledTime}"] = "".join(part.zfill(2) for part in parts[:3])

    prompts = {
        "{patientLastName}": "What's the patient's last name?",
        "{patientFirstName}": "What's the patient's first name?",
        "{patientDOB}": "What's the patient's date of birth (YYYYMMDD)?",
        "{patientGender}": "What's the patient's gender (M/F)",
...
def get_event_time(base_time, offset, duration, event_times):
    """Calculate event time with randomness, handling special cases."""
...
def build_event_messages(template, base_values, duration_min):
    """Generate all 15 HL7 messages with updated PACU timing."""
    base_date = base_values["{YYYYMMDD}"]
    setup_time = base_values["{scheduledTime}"]
    # Updated to parse seconds
    ...
    # Calculate all event times first
    for event, offset in case_events:
        event_time = get_event_time(setup_dt, offset, duration_min, event_times)
        event_times[event] = event_time
    
    # Generate messages using calculated times
...
def main():
    ...
    # Updated help text to indicate seconds
    parser.add_argument("--date", help="Procedure date (YYYYMMDD)")
    parser.add_argument("--time", help="Setup time (HH:MM:SS)")
