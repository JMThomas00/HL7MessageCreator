...
# Default HL7 template with hardcoded AIP segments for staff
default_hl7 = """
...
AIP|1||9941778^{lastName}^{firstName}^W^^^^^EPIC^^^^PROVID|1.1^Primary|GEN|{YYYYMMDD}{scheduledTime}|0|S|{duration}|S
...
class HL7MessageCreator:
    ...
    def update_preview(self):
        """Update the preview with the current patient's S12 message, handling staff names per AIP segment."""
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            template = self.build_template(patient)
            s12_template = "\n".join(line for line in template.splitlines() if not line.startswith("OBX"))
            
            # Staff mappings: segment number -> (last_name, first_name)
            staff_mappings = {
                1: (patient['base_vars'].get('primary_last', tk.StringVar(value='')).get() or '{lastName}',
                    patient['base_vars'].get('primary_first', tk.StringVar(value='')).get() or '{firstName}'),
                2: (patient['base_vars'].get('circ_last', tk.StringVar(value='')).get() or '{lastName}',
                    patient['base_vars'].get('circ_first', tk.StringVar(value='')).get() or '{firstName}'),
                3: (patient['base_vars'].get('scrub_last', tk.StringVar(value='')).get() or '{lastName}',
                    patient['base_vars'].get('scrub_first', tk.StringVar(value='')).get() or '{firstName}'),
                4: (patient['base_vars'].get('crna_last', tk.StringVar(value='')).get() or '{lastName}',
                    patient['base_vars'].get('crna_first', tk.StringVar(value='')).get() or '{firstName}'),
                5: (patient['base_vars'].get('anes_last', tk.StringVar(value='')).get() or '{lastName}',
                    patient['base_vars'].get('anes_first', tk.StringVar(value='')).get() or '{firstName}'),
            }
            
            # Process template line-by-line for AIP segments
            preview_lines = []
            for line in s12_template.splitlines():
                if line.startswith('AIP|'):
                    segment_num = int(line.split('|')[1])
                    if segment_num in staff_mappings:
                        last_name, first_name = staff_mappings[segment_num]
                        updated_line = line.replace('{lastName}', last_name).replace('{firstName}', first_name)
                        preview_lines.append(updated_line)
                    else:
                        preview_lines.append(line)
                else:
                    preview_lines.append(line)
            
            # Join lines and handle other base variables
            preview_text = "\n".join(preview_lines)
            base_values = {k: v.get() if v.get() else k for k, v in patient['base_vars'].items() if k not in ['primary_last', 'primary_first', 'circ_last', 'circ_first', 'scrub_last', 'scrub_first', 'crna_last', 'crna_first', 'anes_last', 'anes_first']}
            for key, val in base_values.items():
                preview_text = preview_text.replace(f'{{{key}}}', val)
            
            self.preview_text.delete(1.0, tk.END)
            self.preview_text.insert(tk.END, preview_text)

    def build_template(self, patient):
        """Build the HL7 template with procedure blocks and additional staff AIP segments."""
        template = default_hl7
        procedures = patient['procedures']
        staff_members = patient['staff_members']
        
        # Handle additional procedures
        ...
        # Handle additional staff members
        for staff in staff_members:
            staff_values = {k: v.get() for k, v in staff.items() if v.get()}
            template = self.add_staff_segment(template, staff_values)
    ...
    def add_staff_segment(self, template, staff_values):
        """Append a new AIP segment for additional staff."""
        lines = template.splitlines()
    ...
    def build_event_messages(self, template, base_values, duration_min):
        ...
        if base_date == "{YYYYMMDD}" or setup_time == "{scheduledTime}" or not base_date or not setup_time:
            # Use template as-is for S12 message
...
base_prompts = [
    ...
    {"key": "primary_last", "prompt": "Primary Surgeon's last name"},
    {"key": "primary_first", "prompt": "Primary Surgeon's first name"},
    {"key": "circ_last", "prompt": "Circulator's last name"},
    {"key": "circ_first", "prompt": "Circulator's first name"},
    {"key": "scrub_last", "prompt": "Scrub's last name"},
    {"key": "scrub_first", "prompt": "Scrub's first name"},
    {"key": "crna_last", "prompt": "CRNA's last name"},
    {"key": "crna_first", "prompt": "CRNA's first name"},
    {"key": "anes_last", "prompt": "Anesthesiologist's last name"},
    {"key": "anes_first", "prompt": "Anesthesiologist's first name"},
]
