...
class HL7MessageCreator:
    def __init__(self, root):
        ...
        self.procedure_frames = []  # Track procedure frames for removal
        ...
        # Add procedure, remove procedure, and staff buttons
        ...
        self.remove_procedure_button = tk.Button(self.content_frame, text="Remove Last Procedure", command=self.remove_last_procedure, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A")
        self.remove_procedure_button.pack(pady=5)
    ...
    def setup_procedure_browser(self):
        ...
        self.tree.bind("<<TreeviewSelect>>", self.update_select_button_state)
        ...
        self.select_procedure_button = tk.Button(button_frame, text="Select Procedure", command=self.select_procedure_button, bg=BG_COLOR, fg=TEXT_COLOR, activebackground="#3A3C5A", state="disabled")
        self.select_procedure_button.pack(side=tk.LEFT, padx=5)
    ...
    def update_select_button_state(self, event):
        """Enable/disable Select Procedure button based on Treeview selection."""
        selected_item = self.tree.selection()
        self.select_procedure_button.config(state="normal" if selected_item else "disabled")
    ...
    def select_procedure_button(self):
        """Handle Select Procedure button click."""
        selected_item = self.tree.selection()
        if selected_item:
            item = self.tree.item(selected_item)
            values = item.get("values")
            if values and len(values) == 5:
                self.select_procedure(None, values)

    def select_procedure(self, event, values=None):
        """Handle procedure selection from Treeview or button."""
        if values is None:
            selected_item = self.tree.selection()
        ...
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            # Check if base procedure is empty
            if not patient["base_vars"]["{procedure}"].get():
                ...
                # {locationDepartment} and {locationOR} remain empty
                patient["procedures"].append(proc)
    ...
    def choose_random_procedure(self):
        ...
        if all_procs and 0 <= self.current_patient_index < len(self.patients):
            ...
            if not patient["base_vars"]["{procedure}"].get():
                ...
                # Add new procedure
                new_proc = {field['key']: tk.StringVar(value='') for field in procedure_fields}
                new_proc["{procedure}"].set(proc["name"])
                new_proc["{procedureId}"].set(proc["id"])
                new_proc["{procedureDescription}"].set(proc["description"])
                new_proc["{specialNeeds}"].set(proc["special_needs"])
    ...
    def update_button_states(self):
        """Update Previous, Next, and Remove Last Procedure button states."""
        ...
        # Update Remove Last Procedure button
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            self.remove_procedure_button.config(
                state="normal" if patient["procedures"] else "disabled",
                fg=TEXT_COLOR if patient["procedures"] else DITHERED_TEXT
            )
