...
class HL7MessageCreator:
    def __init__(self, root):
        ...
        self.procedure_frames = []  # Track procedure frames for UI management
    ...
    def apply_procedure_selection(self, proc_name, proc_id, proc_desc, proc_needs, proc_cpt):
        """Apply procedure selection to base_vars or latest procedure."""
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            # If base procedure is empty or no additional procedures, populate base_vars
            if not patient["base_vars"]["{procedure}"].get() and not patient["procedures"]:
                ...
                # If additional procedures exist, populate the last one
                if patient["procedures"]:
                    ...
                    # Create a new procedure if base is filled but no additional procedures
                    new_proc = {
    ...
    def add_procedure(self):
        """Add a new procedure with placeholder values."""
        proc = {
    ...
    def add_procedure_fields(self, proc):
        """Add entry fields for a procedure and update preview."""
