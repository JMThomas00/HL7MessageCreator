...
class HL7MessageCreator:
    ...
    def choose_random_procedure(self):
        ...
        if all_procs and 0 <= self.current_patient_index < len(self.patients):
            ...
            patient = self.patients[self.current_patient_index]
            if patient["procedures"]:
                proc_vars = patient["procedures"][-1]
            else:
                proc_vars = patient["base_vars"]
            proc_vars["{procedure}"].set(proc["name"])
            proc_vars["{procedureId}"].set(proc["id"])
            proc_vars["{procedureDescription}"].set(proc["description"])
            proc_vars["{specialNeeds}"].set(proc["special_needs"])
            proc_vars["{cptCode}"].set(proc["cpt"])
            self.update_preview()
    ...
    def setup_staff_fields(self):
        ...
        roles = ["Primary Surgeon", "Circulator", "Scrub", "ANE CRNA", "Anesthesiologist"]
        self.staff_vars = []
        for role in roles:
    ...
    def create_new_patient(self):
        ...
        patient = {
            ...
            'staff_members': [],
            'fixed_staff': [
                {"role": "Primary Surgeon", "lastName": tk.StringVar(), "firstName": tk.StringVar()},
    ...
    def update_preview(self):
        ...
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            # Map fixed staff inputs to AIP segments
            for staff in patient['fixed_staff']:
                role = staff['role']
                last_name = staff['lastName'].get()
                first_name = staff['firstName'].get()
                if role == "Primary Surgeon":
                    base_values['{primaryLastName}'] = last_name or '{primaryLastName}'
                    base_values['{primaryFirstName}'] = first_name or '{primaryFirstName}'
                elif role == "Circulator":
                    base_values['circulatorLastName'] = last_name or '{lastName}'
                    base_values['circulatorFirstName'] = first_name or '{firstName}'
                elif role == "Scrub":
                    base_values['scrubLastName'] = last_name or '{lastName}'
                    base_values['scrubFirstName'] = first_name or '{firstName}'
                elif role == "ANE CRNA":
                    base_values['crnaLastName'] = last_name or '{lastName}'
                    base_values['crnaFirstName'] = first_name or '{firstName}'
                elif role == "Anesthesiologist":
                    base_values['anesthesiologistLastName'] = last_name or '{lastName}'
                    base_values['anesthesiologistFirstName'] = first_name or '{firstName}'
            ...
            # Replace {lastName} and {firstName} in AIP segments with staff inputs
            lines = preview_text.splitlines()
            for i, line in enumerate(lines):
                if line.startswith("AIP|2|"):
                    lines[i] = line.replace("{lastName}", base_values.get('circulatorLastName', '{lastName}')).replace("{firstName}", base_values.get('circulatorFirstName', '{firstName}'))
                elif line.startswith("AIP|3|"):
                    lines[i] = line.replace("{lastName}", base_values.get('scrubLastName', '{lastName}')).replace("{firstName}", base_values.get('scrubFirstName', '{firstName}'))
                elif line.startswith("AIP|4|"):
                    lines[i] = line.replace("{lastName}", base_values.get('crnaLastName', '{lastName}')).replace("{firstName}", base_values.get('crnaFirstName', '{firstName}'))
                elif line.startswith("AIP|5|"):
                    lines[i] = line.replace("{lastName}", base_values.get('anesthesiologistLastName', '{lastName}')).replace("{firstName}", base_values.get('anesthesiologistFirstName', '{firstName}'))
            preview_text = "\n".join(lines)
    ...
    def build_template(self, patient):
        ...
        for i, staff in enumerate(staff_members, start=6):
    ...
    def duplicate_procedure_segments(self, template, proc_num, proc_values):
        """Duplicate AIS through AIL segments only."""
        ...
        insert_idx = next(i for i, line in enumerate(lines) if line.startswith("AIP|1|"))
        ...
        for patient in self.patients:
            if 'messages' in patient and patient['messages']:
                ...
                for msg, idx in patient['messages']:
                    ...
                    for i, line in enumerate(lines):
                        if line.startswith("AIP|2|"):
                            lines[i] = line.replace("{lastName}", patient['fixed_staff'][1]['lastName'].get() or '{lastName}').replace("{firstName}", patient['fixed_staff'][1]['firstName'].get() or '{firstName}')
                        elif line.startswith("AIP|3|"):
                            lines[i] = line.replace("{lastName}", patient['fixed_staff'][2]['lastName'].get() or '{lastName}').replace("{firstName}", patient['fixed_staff'][2]['firstName'].get() or '{firstName}')
                        elif line.startswith("AIP|4|"):
                            lines[i] = line.replace("{lastName}", patient['fixed_staff'][3]['lastName'].get() or '{lastName}').replace("{firstName}", patient['fixed_staff'][3]['firstName'].get() or '{firstName}')
                        elif line.startswith("AIP|5|"):
                            lines[i] = line.replace("{lastName}", patient['fixed_staff'][4]['lastName'].get() or '{lastName}').replace("{firstName}", patient['fixed_staff'][4]['firstName'].get() or '{firstName}')
                    msg = "\n".join(lines)
