...
class HL7MessageCreator:
    def __init__(self, root):
        ...
        self.procedure_frame = tk.Frame(self.main_container, bg=BG_COLOR, width=20)
        ...
        self.procedure_frame_width = 100
        self.update_button_states()

    def setup_procedure_browser(self):
        """Set up the procedure browser panel with treeview and functional horizontal scrollbar."""
        # Search bar
        ...
        tree_container = tk.Frame(self.procedure_frame, bg=BG_COLOR)
        tree_container.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.tree = ttk.Treeview(tree_container, show="tree", columns=("name",))
        self.tree.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        self.tree.column("name", width=160, stretch=False)  # Large width to allow overflow
        scrollbar = ttk.Scrollbar(tree_container, orient=tk.HORIZONTAL, command=self.tree.xview)
    ...
    def calculate_treeview_width(self, texts):
        """Calculate the maximum width needed for the treeview text."""
        if not texts:
            return 200  # Default width for empty treeview

        # Get the treeview's font
        style = ttk.Style()
        font_name = style.lookup("Treeview", "font") or "TkDefaultFont"
        font = tkfont.Font(family=font_name)

        # Measure each text item, adding indentation for hierarchy
        max_width = 0
        for text, level in texts:
            # Add 10px indentation per level (specialty=0, category=1, procedure=2)
            indent = level * 10
            text_width = font.measure(text) + indent
            max_width = max(max_width, text_width)

        # Add 20px padding for borders and comfort
        return min(max(200, max_width + 20), 400)

    def populate_procedure_tree(self, filter_text=""):
        """Populate the Treeview with procedures, recursively filtering, and adjust frame width."""
        # Clear existing items
        for item in self.tree.get_children():
            self.tree.delete(item)

        filter_text = filter_text.lower()
        text_items = []  # List of (text, level) for width calculation
                ...
                for proc in procedures[spec][cat]:
                    ...
                    if not filter_text or (
                        ...
                        if not spec_has_match:
                            spec_id = self.tree.insert("", tk.END, text=spec, values=(spec,))
                            text_items.append((spec, 0))  # Specialty level
                            spec_has_match = True
                        if not cat_has_match:
                            cat_id = self.tree.insert(spec_id, tk.END, text=cat, values=(cat,))
                            text_items.append((cat, 1))  # Category level
                            cat_has_match = True
                        self.tree.insert(cat_id, tk.END, text=proc_text, values=(proc_text, proc["name"], proc["id"], proc["description"], proc["special_needs"]))
                        text_items.append((proc_text, 2))  # Procedure level

        # Calculate and set frame width
        self.procedure_frame_width = self.calculate_treeview_width(text_items)
    ...
    def select_procedure(self, event):
        ...
        if selected_item:
            ...
            if values and len(values) >= 4:
                proc_name, proc_id, proc_desc, proc_needs = values[1:5]
    ...
    def toggle_procedure_browser(self):
        """Toggle the procedure browser panel with 5px padding."""
        if self.procedure_panel_visible:
            self.procedure_frame.pack_forget()
            self.procedure_panel_visible = False
        else:
            self.procedure_frame.configure(width=self.procedure_frame_width)
            self.procedure_frame.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 5), pady=0)
    ...
    def duplicate_procedure_segments(self, template, proc_num, proc_values):
        """Duplicate AIS through AIP5 segments."""
...
base_prompts = [
    ...
    {"key": "{primaryLastName}", "prompt": "What's the primary surgeon's last name?"},
    {"key": "{primaryFirstName}", "prompt": "What's the primary surgeon's first name?"},
]
