...
TITLE_CREATOR = "#7DCAE3"  # Light blue for "Creator" title
TITLE_EDITOR = "#7DCAE3"  # Light blue for Editor
DITHERED_TEXT = "#808080"  # Gray for disabled elements
MATCH_BG = "#465BE7"  # Blue for matching procedures
SELECTED_MATCH_BG = "#2C3DAA"  # Darker blue for selected match
...
"""
    ...
    def setup_creator(self):
        ...
        # Pinned top frame for top buttons and preview window
        self.top_pinned_frame = tk.Frame(self.creator_content_container, bg=BG_COLOR)
        self.top_pinned_frame.pack(side=tk.TOP, fill=tk.X)

        # Top buttons frame inside pinned top frame
        self.creator_button_frame_top = tk.Frame(self.top_pinned_frame, bg=BG_COLOR)
        ...
        # Preview window inside pinned top frame
        self.creator_preview_text = scrolledtext.ScrolledText(
            self.top_pinned_frame, width=160, height=20, bg=PREVIEW_BG, fg=TEXT_COLOR, insertbackground=TEXT_COLOR, font=DEFAULT_FONT
        )
        self.creator_preview_text.pack(fill=tk.X, pady=(0, 10))

        # Scrollable canvas for entry fields
        ...
        # Base prompts frame inside scrollable content
        ...
        # Procedures frame inside scrollable content
        self.procedures_frame = tk.Frame(self.creator_content_frame, bg=BG_COLOR)
        self.procedures_frame.pack(fill=tk.X, pady=5)

        # Pinned bottom frame for bottom buttons and Create button
        self.bottom_pinned_frame = tk.Frame(self.creator_content_container, bg=BG_COLOR)
        self.bottom_pinned_frame.pack(side=tk.BOTTOM, fill=tk.X)

        # Bottom buttons frame inside pinned bottom frame
        self.creator_button_frame_bottom = tk.Frame(self.bottom_pinned_frame, bg=BG_COLOR)
        ...
        # Create button inside pinned bottom frame
        self.create_button = tk.Button(self.bottom_pinned_frame, text="Create", command=self.create_patient, fg=TEXT_COLOR, bg=BG_COLOR, font=("Arial", 14))
        ...
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            if patient['procedures']:
                ...
                if self.procedure_frames:
                    ...
                    # Update canvas to eliminate gap
