...
import tkinter.font as tkfont  # Added for font metrics
import pandas as pd
import random
import os
import re
...
MATCH_BG = "#2C3DAA"  # Blue for matching procedures
SELECTED_MATCH_BG = "#465BE7"  # Light blue for selected match
...
class HL7MessageApp:
    def __init__(self, root):
        ...
        self.preview_font = tkfont.Font(family="Courier", size=10)  # Font for preview height calculation
...
3. Ensure required CSV files (`procedures.csv`, `staff_names.csv`, `surgeon_names.csv`, `patient_names.csv`) are in the same directory as the script for data population.
...
"""
        ...
        for i, line in enumerate(lines):
            if line.strip().endswith(')') or line.strip().startswith('- ['):
                ...
                if match:
                 ...
                 '#managing-procedures' in line or '#assigning-staff-and-surgeons' in line or '#generating-and-saving-messages' in line or \
    ...
    def setup_creator(self):
        ...
        # Top frame (Preview and Navigation)
        top_frame = tk.Frame(self.creator_content_container, bg=BG_COLOR)
        top_frame.pack(side=tk.TOP, fill=tk.X)

        # Preview frame
        preview_frame = tk.Frame(top_frame, bg=BG_COLOR)
        preview_frame.pack(fill=tk.X, pady=(0, 10))
        self.creator_preview_text = tk.Text(
            preview_frame, width=160, height=15, bg=PREVIEW_BG, fg=TEXT_COLOR,
            font=("Courier", 10), wrap=tk.NONE, insertbackground=TEXT_COLOR
        )
        self.preview_scrollbar = tk.Scrollbar(preview_frame, orient=tk.VERTICAL, command=self.creator_preview_text.yview)
        self.creator_preview_text.config(yscrollcommand=self.preview_scrollbar.set)
        self.creator_preview_text.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # Navigation buttons
        self.creator_button_frame_top = tk.Frame(top_frame, bg=BG_COLOR)
        ...
        dummy_var = tk.StringVar(value="Scheduled & Case Events")
        options = ["Scheduled", "Scheduled & Case Events", "Scheduled & Canceled"]
        for option in options:
            rb = tk.Radiobutton(message_type_frame, text=option, variable=dummy_var, value=option, bg=BG_COLOR, fg=TEXT_COLOR, selectcolor=PREVIEW_BG, font=DEFAULT_FONT)
            rb.pack(side=tk.LEFT, padx=5)
            self.message_type_radios.append(rb)

        # Middle frame (Scrollable entry fields)
        middle_frame = tk.Frame(self.creator_content_container, bg=BG_COLOR)
        middle_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        self.middle_canvas = tk.Canvas(middle_frame, bg=BG_COLOR, highlightthickness=0)
        self.middle_canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.entry_content_frame = tk.Frame(self.middle_canvas, bg=BG_COLOR)
        self.middle_canvas.create_window((0, 0), window=self.entry_content_frame, anchor="nw")
        self.entry_content_frame.bind("<Configure>", lambda e: self.middle_canvas.configure(scrollregion=self.middle_canvas.bbox("all")))
        self.middle_canvas.bind_all("<MouseWheel>", lambda e: self.middle_canvas.yview_scroll(int(-1 * (e.delta / 120)), "units"))

        # Base prompts
        self.base_prompts_frame = tk.Frame(self.entry_content_frame, bg=BG_COLOR)
        ...
        # Procedures frame
        self.procedures_frame = tk.Frame(self.entry_content_frame, bg=BG_COLOR)
        self.procedures_frame.pack(fill=tk.X, pady=5)

        # Bottom frame (Action buttons and Create)
        bottom_frame = tk.Frame(self.creator_content_container, bg=BG_COLOR)
        bottom_frame.pack(side=tk.BOTTOM, fill=tk.X)

        self.creator_button_frame_bottom = tk.Frame(bottom_frame, bg=BG_COLOR)
        ...
        self.create_button = tk.Button(bottom_frame, text="Create", command=self.create_patient, fg=TEXT_COLOR, bg=BG_COLOR, font=("Arial", 14))
    ...
    def random_patient_full(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            patient["base_vars"]["{patientFirstName}"].set(name["First Name"])
            patient["base_vars"]["{patientLastName}"].set(name["Last Name"])
            patient["base_vars"]["{patientGender}"].set(random.choice(["M", "F"]))
            self.random_dob()
            self.last_mrn += 1
            patient["base_vars"]["{patientMRN}"].set(f"MRN{self.last_mrn:06d}")
            patient["base_vars"]["{YYYYMMDD}"].set(datetime.now().strftime("%Y%m%d"))
            patient["base_vars"]["{scheduledTime}"].set(datetime.now().strftime("%H%M%S"))
            patient["base_vars"]["{duration}"].set(str(random.randint(60, 120)))
            self.random_surgeon()
            self.random_staff()
            self.choose_random_procedure()
            self.creator_update_preview()

    def clear_all(self):
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            for key in patient["base_vars"]:
                patient["base_vars"][key].set("")
            patient["base_vars"]["{encounterType}"].set("IP")
            patient["procedures"].clear()
            patient["staff_members"].clear()
            patient["additional_surgeons"].clear()
            patient["messages"].clear()
            patient["procedure_specialty"].set("GEN")
            patient["message_type"].set("Scheduled & Case Events")
            ...
            self.procedure_frames = []
            self.creator_update_preview()
            self.update_middle_scroll_region()

    def random_surgeon(self):
        if 0 <= self.current_patient_index < len(self.patients):
            patient = self.patients[self.current_patient_index]
            if not self.staff_entries["Primary Surgeon"]["lastName"].get() and not self.staff_entries["Primary Surgeon"]["firstName"].get():
            ...
            for additional_surgeon in patient['additional_surgeons']:
                if not additional_surgeon["lastName"].get() and not additional_surgeon["firstName"].get():
                    surgeon = self.surgeon_names.sample(1).iloc[0]
                    additional_surgeon["firstName"].set(surgeon["First Name"])
                    additional_surgeon["lastName"].set(surgeon["Last Name"])
                    additional_surgeon["id"].set(str(surgeon["ID"]))
            self.creator_update_preview()
    ...
    def random_staff(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            for role in ["Circulator", "Scrub", "CRNA", "Anesthesiologist"]:
                if not self.staff_entries[role]["lastName"].get() and not self.staff_entries[role]["firstName"].get():
            ...
            for staff_member in patient['staff_members']:
                if not staff_member["lastName"].get() and not staff_member["firstName"].get():
                    staff = self.staff_names.sample(1).iloc[0]
    ...
    def remove_last_surgeon(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            if patient['additional_surgeons']:
                ...
                self.staff_group_frame.update_idletasks()
            self.creator_update_preview()
            self.update_middle_scroll_region()
    ...
    def add_staff_fields(self, staff):
        row = len(self.fixed_roles) + len(self.additional_surgeons) + len(self.additional_staff)
        row_frame = tk.Frame(self.staff_group_frame, bg=BG_COLOR)
    ...
    def add_surgeon_fields(self, surgeon):
        row = len(self.fixed_roles) + len(self.additional_surgeons)
        row_frame = tk.Frame(self.staff_group_frame, bg=BG_COLOR)
        row_frame.grid(row=row, column=0, sticky="w", pady=2)
    ...
    def update_middle_scroll_region(self):
        self.entry_content_frame.update_idletasks()
    ...
    def creator_update_preview(self):
        if 0 <= self.current_patient_index < len(self.patients):
            ...
            # Adjust preview height dynamically
            num_lines = int(self.creator_preview_text.index(tk.END).split('.')[0]) - 1
            linespace = self.preview_font.metrics("linespace")
            max_lines = 400 // linespace  # Approx 20 lines for 400 pixels
            if num_lines <= 0:
                self.creator_preview_text.config(height=15)  # ~300 pixels
                self.preview_scrollbar.pack_forget()
            elif num_lines <= max_lines:
                self.creator_preview_text.config(height=num_lines)
                self.preview_scrollbar.pack_forget()
            else:
                self.creator_preview_text.config(height=max_lines)
    ...
    def editor_prev_message(self):
        if self.current_message_index > 0:
            ...
            self.current_patient_index -= 1
            self.current_message_index = len(self.patient_blocks[self.current_patient_index]['messages']) - 1
            self.editor_load_message()
    ...
    def editor_next_message(self):
        if self.current_message_index < len(self.patient_blocks[self.current_patient_index]['messages']) - 1:
            self.current_message_index += 1
            self.editor_load_message()
        elif self.current_patient_index < len(self.patient_blocks) - 1:
            self.current_patient_index += 1
            self.current_message_index = 0
            self.editor_load_message()
...
base_prompts = [
    ...
    {"prompt": "Duration (min):", "key": "{duration}"},
