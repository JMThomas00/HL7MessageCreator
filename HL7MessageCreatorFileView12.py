...
class HL7MessageApp:
    ...
    def build_event_messages(self, template, base_values, duration_min):
        # Create S12 template without OBX segments
        s12_template = "\n".join(line for line in template.splitlines() if not line.startswith("OBX"))
        
        # Prepare S12 message with base values (placeholders or actual values)
        s12_replacements = base_values.copy()
        s12_replacements["{eventTime}"] = "{eventTime}"  # Placeholder for eventTime
        ...
        if base_date == "{YYYYMMDD}" or setup_time == "{scheduledTime}":
            # Fallback mode: Use placeholders for date and time
            for i, (event_name, _) in enumerate(case_events):
                ...
                event_replacements["{YYYYMMDD}"] = "{YYYYMMDD}"
            ...
            try:
                ...
                # If date/time are invalid, fallback to placeholder mode
                for i, (event_name, _) in enumerate(case_events):
                    event_replacements = base_values.copy()
                    event_replacements["{caseEvent}"] = event_name
                    event_replacements["{eventTime}"] = "{eventTime}"
